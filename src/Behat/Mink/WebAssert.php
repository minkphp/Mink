...
class WebAssert
{
    ...
    {
        $actual = $this->session->getResponseHeader($name);
        $message = sprintf('Current response header %s is "%s", but should be %s.', $name, $actual, $value);
    ...
    public function responseHeaderNotContains($name, $value)
    {
        ...
        $actual = preg_replace('/\s+/u', ' ', $actual);
        $regex  = '/'.preg_quote($value, '/').'/ui';
        $message = sprintf('The text "%s" was found in the %s response header.', $value, $name);
